...
if (empty($csrfResponse)) {
    ...
    try {
        $sqlSelect = 'SELECT * FROM register_form2 WHERE org_id_root = :org_id ';
    ...
    if (count($result) == 0) {
        ...
        org.* FROM register_form2
        LEFT JOIN  org
            ON org.org_id = register_form2.org_id_root
        WHERE register_form2.org_id_root  = :org_id ";
                $stm = $sqlConn->conn->prepare($sql);
