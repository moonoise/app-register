...
class Grade extends SqlConn 
{
    ...
    function required_subject($subjectId) {  //   เช็ดว่ามีวิชาต่อเนื่องไหม return count , data
        $result = array();
        $arr = array();
        try {
    ...
    function std_grade_check($stdId , $subjectId , $arrGrade ) {  //ค้นหาเกรด ที่ต้องการ - A,B+,B,C+,C,D+,D , F,W
    ...
    function required_subject_check($stdId , $subjectId ,$year) {  // ตรวจสอบ ว่ารายวิชาที่จะลงทะเบียน แบบลงได้หรือได้ 
        
        $conditionFalse = array('false1' => 'ไม่พบการลงทะเบียนเรียน',
                                'false2' => 'เป็นรายวิชาของชั้นปีที่สูงกว่า',
                                'f' => 'ติด F รายวิชา ',
                                'w' => 'เคย DROP ราย '); 
        $conditionTrue = array('true1' => 'ไม่ใช่รายวิชาต่อเนื่อง',
    ...
    function check_level_subject($stdId,$subject_id) { //เช็คว่าเป็นรายวิชาของชั้นปีที่สูงกว่า ที่ตัวเองเรียนหรือไม่ true = ไม่สูง false = สูงกว่า 
        $std = $this->callStudent($stdId);
    ...
    function check_grade($stdId,$subjectId,$arrGrade) {  //  ค้นหาเกรดที่ต้องการ 
        $strGrade = "'".implode("','", $arrGrade)."'";
        $result = array();
        try {
            $sql = "SELECT * FROM student_subject 
